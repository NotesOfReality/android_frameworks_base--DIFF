...
public class PowerTile extends QSTileImpl<BooleanState> {
    ...
    protected void handleUpdateState(BooleanState state, Object arg) {
        if (mTileMode == 1) {
            ...
            if (mRebootMode == 1) {
                state.label = mContext.getString(R.string.quick_settings_power_option_reboot_system);
                state.icon = ResourceIcon.get(R.drawable.ic_qs_reboot);
            } else if (mRebootMode == 2) {
                state.label = mContext.getString(R.string.quick_settings_power_option_reboot_recovery);
                state.icon = ResourceIcon.get(R.drawable.ic_qs_reboot);
            } else if (mRebootMode == 3) {
                state.label = mContext.getString(R.string.quick_settings_power_option_reboot_bootloader);
                state.icon = ResourceIcon.get(R.drawable.ic_qs_reboot);
            }
        } else {
